...
class View(models.Model):
    ...
    def sh_snippet_builder_save_js_assets_snippet(self, arch, snippet_key):
        if arch:
            ...
            attach = attachment_obj.search([("url", "=", url)], limit=1)
...
class sh_snippet_builder(models.Model):
    ...
    js_asset_view_id = fields.Many2one(string="JS Assets View", comodel_name="ir.asset")
    ...
    def create(self, vals):
        ...
        res.write({"js_asset_view_id": asset.id})
        # -------------------------------------------
        # Make Assets template for JS
        # -------------------------------------------
        return res
            ...
            snippet_panel_view = IrUiView.sudo().search(
                [
                    (
                        "key",
                        "=",
                        "sh_snippet_builder.sh_snippet_builder_snippets",
                    )
                ],
                order="id desc",
                limit=1,
