...
class WebsiteBlog(http.Controller):
    ...
    def blog_feed(self, blog, limit='15', **kwargs):
        ...
        v['posts'] = request.env['blog.m4pnews'].search([('blog_id', '=', blog.id)], limit=min(int(limit), 50), order="post_date DESC")
        v['html2plaintext'] = html2plaintext
        r = request.render("mindala.blog_feed", v, headers=[('Content-Type', 'application/atom+xml')])
        return r

    @http.route([
        '''/blog/<model("blog.blog"):blog>/post/<model("blog.m4pnews", "[('blog_id','=',blog.id)]"):blog_post>''',
